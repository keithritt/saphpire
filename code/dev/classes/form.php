...
class Form
{
	public static function _open($sAction, $sTarget = null, $sMethod = 'post')
  {
    $aOptions = array(
      'action' => $sAction,
      'target' => $sTarget,
      'method' => $sMethod);
    return self::open($aOptions);
  }

  public static function open($aOptions)
  {
  ...
  public static function _get_submit_button($sValue = 'Submit')
  {
    return '<input type="submit" value="'.$sValue.'" />';
  }

  public static function _get_dropdown($sName, $aOptions, $vDefault = null, $aAttributes = null)
	{

    return self::get_dropdown(array('name' => $sName, 'options' => $aOptions, 'default' => $vDefault, 'attributes'=> $aAttributes));
	}
    ...
    if(!(isset($aParams['attributes']) && count($aParams['attributes'])))
	...
	public static function _get_hidden($sName, $sValue = null)
	{

    return self::get_hidden(array('name' => $sName, 'value' => $sValue));
	}

	public static function get_hidden($aOptions)
	{
  ...
  public static function _get_integer_textbox($sName, $sClass = null, $sStyle = null)
  {
    return self::get_textbox(array('name' => $sName, 'class' => $sClass, 'style' => $sStyle));
  }

	public static function _get_textbox($sName, $sValue = '', $sClass = null, $sStyle = null)
	{
		return self::get_textbox(array('name' => $sName, 'value' => $sValue, 'class' => $sClass, 'style' => $sStyle));
	}

	public static function get_textbox($aOptions)
	{
		$sId = @Util::coalesce($aOptions['id'], $aOptions['name']);
    $sValue = @Util::coalesce($aOptions['value'], '');
		$sClass = @Util::coalesce($aOptions['class'], 'form-control');
    $sStyle = @Util::coalesce($aOptions['style'], '');
		return '<input id="'.$sId.'" name="'.$aOptions['name'].'" value="'.$sValue.'" class="'.$sClass.'" style="'.$sStyle.'"/>';
	}

	public static function _get_price_textbox($sName = 'price')
	{
		$sId = $sName; // fix later
		return '
  ...
  public static function _get_textarea($sName)
  {
    return self::get_textarea(array('name' => $sName));
  }

  public static function get_textarea($aOptions)
  {
    $sId = @Util::coalesce($aOptions['id'], $aOptions['name']);
    $sClass = @Util::coalesce($aOptions['class'], 'form-control');
    return '<textarea id="'.$sId.'" name="'.$aOptions['name'].'" class="'.$sClass.'" ></textarea>';
  }

  public static function _get_date_picker($sId)
  {
    return self::get_date_picker(array('id' => $sId));
  }
