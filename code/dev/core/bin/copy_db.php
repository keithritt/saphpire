...
class DBCopy
{
	function __construct($sTo, $sFrom)
	{
		...
		$this->aSchemas = Db::get_schemas();

		expose($this->aSchemas);
		//stop();
	...
	function sync_tables()
	{
		...
		foreach($this->aSchemas as $sSchema)
		{
			//pr('Running sync for schema: '.$sSchema);
			//continue;
	...
	function get_table_list($sSchema, $sDirection)
	{
		...
		{
			$aRet[] = $aRow['Tables_in_'.Db::get_db_name($sSchema, $this->aDirections[$sDirection])];
		}
