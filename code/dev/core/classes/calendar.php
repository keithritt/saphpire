...
class Calendar
{
  ...
  private function add_upcoming_content($aEventData)
  {
      ...
      switch($aEvent['interval_id'])
      {
        case TYPE::_INTERVAL_ONE_TIME:
          ...
          if($iTs >= $iStart && $iTs <= $iEnd)
            $aCalcData[TYPE::_INTERVAL_ONE_TIME][$iTs] = '<div class="clickable underlined" onclick="view_event('.$aEvent['id'].')">'.$aEvent['title'].'</div>';
          break;
        case TYPE::_INTERVAL_WEEKLY:
          //expose($aEvent);
          ...
          if($iTs >= $iStart && $iTs <= $iEnd)
            $aCalcData[TYPE::_INTERVAL_WEEKLY][$iTs] = '<div class="clickable underlined" onclick="view_event('.$aEvent['id'].')">'.$aEvent['title'].'</div>';
        ...
        case TYPE::_INTERVAL_MONTHLY:
          ...
          for($iWalk = 0; $iWalk <= 1; $iWalk++)
          {
            ...
            $iTs = Util::strtotime($sDate);
            if($iTs >= $iStart && $iTs <= $iEnd)
              $aCalcData[TYPE::_INTERVAL_MONTHLY][$iTs] = '<div class="clickable underlined" onclick="view_event('.$aEvent['id'].')">'.$aEvent['title'].'</div>';
        ...
        case TYPE::_INTERVAL_YEARLY:
          ...
          $iTs = Util::strtotime($sDate);
          if($iTs >= $iStart && $iTs <= $iEnd)
            $aCalcData[TYPE::_INTERVAL_YEARLY][$iTs] = '<div class="clickable underlined" onclick="view_event('.$aEvent['id'].')">'.$aEvent['title'].'</div>';
          break;
      }
    }
    ...
    if(isset($aCalcData[TYPE::_INTERVAL_WEEKLY]))
    {
      foreach($aCalcData[TYPE::_INTERVAL_WEEKLY] as $iTs => $sDesc)
        $aTmp[$iTs] = $sDesc;
    }

    if(isset($aCalcData[TYPE::_INTERVAL_MONTHLY]))
    {
      foreach($aCalcData[TYPE::_INTERVAL_MONTHLY] as $iTs => $sDesc)
        $aTmp[$iTs] = $sDesc;
    }

    if(isset($aCalcData[TYPE::_INTERVAL_YEARLY]))
    {
      foreach($aCalcData[TYPE::_INTERVAL_YEARLY] as $iTs => $sDesc)
        $aTmp[$iTs] = $sDesc;
    }

    if(isset($aCalcData[TYPE::_INTERVAL_ONE_TIME]))
    {
      foreach($aCalcData[TYPE::_INTERVAL_ONE_TIME] as $iTs => $sDesc)
        $aTmp[$iTs] = $sDesc;
    }
