...
//@TODO - implement a way to use the features of the model class for updates and deletes with out having to run a select/fetch() first
...
class Model implements Iterator
{
   ...
   public function __isset($sLookup)
   {
    ...
    //@TODO - move this logic to a config file - perhaps one this is dynamically created on deployment
    // get a list of core schemas

    $aModels = array();

    foreach(array('core', 'custom') as $sCodeArea)
    {
      //pr($sCodeArea);
      $aSchemas = scandir(CODE_PATH.'/'.$sCodeArea.'/models');
      unset($aSchemas[0], $aSchemas[1]);

      //expose($aSchemas);
      foreach($aSchemas as $sTmpSchema)
      {
        //pr($sTmpSchema);
        $aTables = scandir(CODE_PATH.'/'.$sCodeArea.'/models/'.$sTmpSchema);
        unset($aTables[0], $aTables[1]);

        //expose($aModels);
        foreach($aTables as $sTmpTable)
        {
          //pr($sTmpTable);
          $aModels[$sTmpSchema.'.'.str_replace('.php', '', $sTmpTable)] = $sCodeArea;
        }
    ...
    //stop();
		...
		if(isset($aModels[$sSchema.'.'.$sTable]))
    {
      //line();
      $sCodeEnv = $aModels[$sSchema.'.'.$sTable];
			require_once(CODE_PATH.'/'.$sCodeEnv.'/models/'.$sSchema.'/'.$sTable.'.php');
	...
	public function fetch($vLookup, $vExpected = null, $sOrderBy = null)
	{
    if(self::$bDebugMode)
      hit();
    $vExpected = Util::coalesce($vExpected, 1);
	...
	{
		$this->bAudit = true;
    //pr($this->sSchema.'_'.$this->sTable);
		if(Request::$bDebugMode)
		{
			if(!defined('Type::AUDIT_'.strtoupper($this->sSchema.'_'.$this->sTable)))
			{
				//line();
				Log::error('Model->enable_auditing() without a constant for : Type::AUDIT_'.strtoupper($this->sSchema.'_'.$this->sTable));
				//line();
				stop();
			}
		}
		...
		//foreach($this->aSubModels as $sSubTable => $aSubData)
		{
			...
			//if($sSubTable == $this->sTable)
			{
				...
				foreach($aData as $sCol => $vVal)
				{
					...
					{
						$aColumns[] = $sCol;
						if(is_numeric($vVal) || in_array($vVal, Db::$aKeywords))
								$aValues[] = $vVal;
						else
								$aValues[] = Db::esc($vVal);
					}
				...
				if($this->bAudit)
				{
					...
					$iXrefTypeId = constant('Type::AUDIT_'.strtoupper($this->sSchema.'_'.$this->sTable));
					...
					if($oModel->is_insert())
					{
						...
						$oModel->create_by = Session::get('login||id'); // add option for scripts
					}
					else
					{
						$oModel->update_ts = Db::datetime('now', false);
						$oModel->update_by = Session::get('login||id'); // add option for scripts
					}
	...
	protected function InitSubModel($sSchema, $sTable, $vLookup = null, $vExpected = null)
	{
    if(self::$bDebugMode)
      hit();
		...
		if(isset($vLookup))
			$this->aSubModels[$this->iCursor][$sSchema.'.'.$sTable]->fetch($vLookup, $vExpected);
	}
